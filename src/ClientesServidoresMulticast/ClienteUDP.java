...
public class ClienteUDP {
    public static void main(String args[]) throws IOException {
        ...
        DatagramPacket envelopeAEnviar = new
                DatagramPacket(requisicaoServidor, requisicaoServidor.length, ip,
                5000);
        ...
        while(true) {
            ...
            DatagramPacket envelopeAReceber = new
                    DatagramPacket(respostaDoServidor, respostaDoServidor.length);
            socket.receive(envelopeAReceber);
            String msgRecebida = new
                    String(envelopeAReceber.getData()).trim();
            System.out.println(msgRecebida);
            InetAddress ipUsuario = null;
            if(msgRecebida.charAt(0) == 'i' && msgRecebida.charAt(1)
                    == 'p'){
                ...
                iniciarConversacao(teclado, socket, ipUsuario,
                        envelopeAReceber);
                continue;
            }
            ...
            DatagramPacket envelopeResposta = new
                    DatagramPacket(respostaAoServidor, respostaAoServidor.length, ip,
                    5000);
            ...
            if (resposta.equalsIgnoreCase("x")) {
                ...
                envelopeAReceber = new
                        DatagramPacket(respostaDoServidor, respostaDoServidor.length);
                socket.receive(envelopeAReceber);
                String msgChat = new
                        String(envelopeAReceber.getData()).trim();
                System.out.println("Mensagem recebida de: " +
                        envelopeAReceber.getAddress().getHostAddress() + " | " +
                        envelopeAReceber.getAddress().getHostName() + ": " + msgChat);
    ...
    public static void iniciarConversacao(Scanner teclado,
                                          DatagramSocket socket, InetAddress ip, DatagramPacket dp) throws
            IOException {
        String resposta;
        while(true){
            ...
            DatagramPacket envelopeResposta = new
                    DatagramPacket(respostaAoCliente, respostaAoCliente.length, ip, 5000);
            System.out.println("Enviando mensagem para o usuário: " +
            ...
            dp = new DatagramPacket(respostaDoUsuario,
                    respostaDoUsuario.length);
            socket.receive(dp);
            String msgChat = new String(dp.getData());
            System.out.println("Mensagem recebida de: " +
                    dp.getAddress().getHostAddress() + " | " +
                    dp.getAddress().getHostName() + ": " + msgChat);
        }
        ...
        requisicaoServidor = msg.getBytes();
        DatagramPacket envelopeAEnviar = new
                DatagramPacket(requisicaoServidor, requisicaoServidor.length,
                InetAddress.getByName("25.74.5.8"), 5000);
        socket.send(envelopeAEnviar);
    }
}